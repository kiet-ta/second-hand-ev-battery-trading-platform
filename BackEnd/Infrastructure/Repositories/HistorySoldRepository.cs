...
#pragma warning disable CS8601
        ...
        public async Task<List<BatteryItemDto>> MapToBatteryItemsAsync(IQueryable<Item> batteryItemsQuery)
        {
            ...
            var query = from item in batteryItemsQuery // Dùng IQueryable được truyền vào
                        ...
                        let isSold = _context.PaymentDetails.Any(pd_s => pd_s.ItemId == item.ItemId && _context.Payments.Any(p_s => p_s.PaymentId == pd_s.PaymentId && p_s.Status == PaymentStatus.Completed_PaymentStatus.ToString()))
                        ...
                        let isPending = !isSold && !isProcessing && _context.PaymentDetails.Any(pd_pe => pd_pe.ItemId == item.ItemId && _context.Payments.Any(p_pe => p_pe.PaymentId == pd_pe.PaymentId && p_pe.Status == PaymentStatus.Pending_PaymentStatus.ToString()))
