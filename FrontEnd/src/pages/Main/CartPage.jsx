...
function CartPage() {
      ...
      if (orderItemsResponse && orderItemsResponse.length > 0) {
        ...
        const itemsWithSeller = await Promise.all(
          uniqueOrderItems.map(async (orderItem) => {
            ...
            let sellerName = "Cửa hàng EV & Pin"; // default store
            if (detail.updatedBy) {
              try {
                const seller = await userApi.getUserByID(detail.updatedBy);
                sellerName = seller.fullName || sellerName;
              } catch {
                sellerName = "Cửa hàng EV & Pin";
              }
            }
        ...
        combinedCartData = itemsWithSeller.filter(Boolean);
  ...
  // Group by store
  const groupedStores = [...new Set(cartItems.map(i => i.store))];
        ...
        <div className="bg-white shadow-sm mb-4">
          {cartItems.length > 0 ? (
            groupedStores.map((store) => {
              const items = cartItems.filter(i => i.store === store);
              return (
                <div key={store} className="mb-6">
                  <div className="p-4 font-semibold text-[#2C2C2C] border-b border-[#E8E4DC] flex items-center">
                    <input
                      ...
                      onChange={() => {
                        const allSelected = items.every(i => selectedItemIds.includes(i.id));
                        if (allSelected) {
                          setSelectedItemIds(prev => prev.filter(id => !items.some(it => it.id === id)));
                        } else {
                          setSelectedItemIds(prev => [...prev, ...items.map(it => it.id).filter(id => !prev.includes(id))]);
                        }
                      }}
                      checked={items.every(i => selectedItemIds.includes(i.id))}
                    />
                    {store}
                  </div>
